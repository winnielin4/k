...
let split_config (config: k) : k =
  ...
  [Thread(first_global,first_thread_id,first_thread,[Map(sort,lbl,first_other_threads)])])
...
let rec take_steps (active_threads: k list) (waiting_threads: k list) (config: k) (depth: int) (n: int) (last_resort: bool) : k * int =
  if n = depth then (
    ...
    | Some config -> (
      take_steps active_threads waiting_threads config depth (n+1) false
    )
    | None -> (
...
let run (config: k) (depth: int) : k * int =
  let first_config = split_config config in
  let last_config,n = take_steps [[Int (Z.of_int 0)]] [] first_config depth 0 false in
